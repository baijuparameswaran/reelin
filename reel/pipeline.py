...
def _summarize_casting(r: dict) -> str:
    ...
    for c in r.get("casting", []):
        rows.append(f"  · {c.get('name','?')}: {c.get('casting_brief','?')[:70]}")
        pf = c.get("physical_form", "")
        if pf:
            rows.append(f"      {pf[:80]}")
    return "\n".join(rows) or "  (none)"
...
def _render_casting_images(casting: dict, out: Path) -> int:
    """Best-effort: render a basic portrait per character from its visual_prompt,
    storing the relative path back on each casting entry. Idempotent (skips
    portraits already on disk) so it's cheap on --resume. Returns count with art.
    """
    ...
    for c in casting.get("casting", []):
        img = cast_dir / f"{_slug(c.get('name', 'character'))}.png"
        if img.exists():
            c["image_path"] = str(img.relative_to(out))
            n += 1
            continue
        prompt = c.get("visual_prompt") or c.get("physical_form") or c.get("name", "")
        if prompt and imagegen.generate_image(prompt, img):
            c["image_path"] = str(img.relative_to(out))
            n += 1
    return n
